...
void Motor::playVibAlert( uint8_t waveform, uint8_t pwr, uint8_t onTime, uint8_t offTime )
{
	...
	uint8_t pwr_range = currentMotor.max_duty - currentMotor.min_duty;
	uint16_t currentPWR;
	// Setup PWM for 31.25kHz and specified % duty.
	TCCR1A |= (1<<COM1A1) | (1<<WGM11);   // 9 bit fast  PWM, not inverted, output on OC1A (D9) A.K.A. PWM_OUT
	TCCR1B |= (1<<WGM12) | (1<<CS10);    //  9 bit fast PWM, prescaler /1
	OCR1A = 0;
	...
	while( !stopPlaying )
	{
		...
		{        
			...
			OCR1A = currentPWR;	// OCR1 is 0-511 
